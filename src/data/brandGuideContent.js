...
export const DEFAULT_BRAND_GUIDE_CONTENT = {
  ...
  platform: {
    ...
    body: 'Users often search different brand spellings before they reach the right site. On this project, BpxPro is the main public brand and the aliases BPX, BPEXCH, BPXPRO, BettPro and Bett Pro are treated as the same platform identity.',
    highlights: [
...
const BRAND_GUIDE_CONTENT_STORAGE_KEY = 'flowexch.brandGuide.content.v1'

function loadCachedBrandGuideContent() {
  if (typeof localStorage === 'undefined') return null
  try {
    const raw = localStorage.getItem(BRAND_GUIDE_CONTENT_STORAGE_KEY)
    if (!raw) return null
    return normalizeBrandGuideContent(JSON.parse(raw))
  } catch {
    return null
  }
}

function saveCachedBrandGuideContent(content) {
  if (typeof localStorage === 'undefined') return
  try {
    localStorage.setItem(BRAND_GUIDE_CONTENT_STORAGE_KEY, JSON.stringify(content))
  } catch {
    /* ignore */
  }
}

let brandGuideContentCache =
  loadCachedBrandGuideContent() || normalizeBrandGuideContent(DEFAULT_BRAND_GUIDE_CONTENT)

export function getBrandGuideContent() {
  return brandGuideContentCache
}

export function setBrandGuideContentCache(content) {
  brandGuideContentCache = normalizeBrandGuideContent(content)
  saveCachedBrandGuideContent(brandGuideContentCache)
  return brandGuideContentCache
}

export async function loadBrandGuideContent() {
  try {
    const data = await fetchBrandGuideContent()
    setBrandGuideContentCache(data?.content || data)
  } catch (err) {
    console.warn('Brand guide content load failed, using defaults:', err.message)
  }
  return brandGuideContentCache
}
